...
class Model extends BaseModel{
	...
	initData()
	{
		...
		if(qzmjLogicInstance != null)
		{
			...
			this.handcards=g_deepClone(qzmjLogicInstance.roundSettle.handcards);
		}
	}
	setAsViewAgain(){
...
class View extends BaseView{
	...
	initUi()
	{
		...
		if(QzmjLogic.getInstance().bViewMode){
	...
	showTotalloseWin(logicseatid)
	{
		...
		if (playerSettleInfo.winflag == 1) {
		    ...
		    let self = this;
			cc.loader.loadRes(`Mahjong/Textures/settle/hu${playerSettleInfo.hutime}`, cc.SpriteFrame, function (err, spriteFrame) {
			    if (err) {
			        cc.error(err.message || err);
			        return;
			    }
...
//c, 控制
@ccclass
export default class QzmjSettleCtrl extends BaseCtrl {
