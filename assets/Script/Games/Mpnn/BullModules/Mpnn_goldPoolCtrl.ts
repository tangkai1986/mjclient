...
//c, 控制
@ccclass
export default class Mpnn_goldPoolCtrl extends BaseCtrl {
    view:View = null
    model:Model = null
    ...
    test(){
        this.view.flyGold(cc.p(-522,-278), cc.p(2,-173), null, ()=>{
            ...
            // MpnnLogic.getInstance().emit_flyGoldEnd(tagStr);
        });
    ...
    */
    onSettle_bull(msg){
        ...
        //飞行金币
        //因为庄家获胜不在消息的赢家列表中，判断庄家是否获胜
        if(!msg.winIdList){
            return
        }
        if(msg.scoreInfo[MpnnLogic.getInstance().getDelaerSeatId()]>0){
            msg.winIdList.push(MpnnLogic.getInstance().getDelaerSeatId().toString())
        }
        for(let i = 0;i<msg.winIdList.length;i++){
            let winSeatId = msg.winIdList[i];
            let winViewSeatId = RoomMgr.getInstance().getViewSeatId(winSeatId);
            let startPosList = [];
    ...
    playGetGoldEffect (winLogicId){
        ...
        if(this.dict_anis[winLogicId]){
            ...
            MpnnLogic.getInstance().openGoldFalsh((curPrefab, curNode)=>{
