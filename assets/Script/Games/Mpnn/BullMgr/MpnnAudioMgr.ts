...
//通比牛牛音效管理器
let processaudiodic={};
processaudiodic[MpnnConst.process.start]='audio_ready'; //开始 
processaudiodic[MpnnConst.process.giveCards]='audio_fapai'; //发牌
processaudiodic[MpnnConst.process.settle]='kaijin'; //结算

export default class MpnnAudioMgr extends BaseMgr {
    ...
    constructor(){
        super();
        this.baseUrl = cc.url.raw("resources/audio/Games/Mpnn/%s.mp3");
        this.womanUrl = cc.url.raw("resources/audio/Games/Mpnn/pth/woman/%s.mp3");
        this.manUrl = cc.url.raw("resources/audio/Games/Mpnn/pth/man/%s.mp3")
        //女声结果
   ...
   //播放结果语音
    ...
    //单例
    private static _instance:MpnnAudioMgr;
    public static getInstance ():MpnnAudioMgr{
        if(!this._instance){
            this._instance = new MpnnAudioMgr();
        }
