//mpnn属性配置,包含算分，以及各种事件开关
export default class MpnnProp {
    ...
    private defaultcfg={
        ...
        v_playeraddin:0,//闲家推注的倍数 
        v_grabbanker:1,//最大抢庄的倍数
        v_playerbuyLimit:1,//是否闲家买码
        v_betLimit:1,//是否下注限制
        v_betredoubleLimit:1,//是否下注加倍
        v_shufflingLimit:1,//是否禁止搓牌
    };
    ...
    //获取闲家推注的倍数 如果有用户自定义的闲家推注的倍数 ,就用用户自定义的闲家推注的倍数 
    get_v_playeraddin(){
        if(this.customcfg['v_playeraddin']!=null)
        {
          return this.customcfg['v_playeraddin'];
        }
        return this.defaultcfg['v_playeraddin'];
    }
    //获取最大抢庄的倍数如果有用户自定义的最大抢庄的倍数,就用用户自定义的最大抢庄的倍数
    get_v_grabbanker(){
        if(this.customcfg['v_grabbanker']!=null)
        {
          return this.customcfg['v_grabbanker'];
        }
        return this.defaultcfg['v_grabbanker'];
    }
    //获取是否闲家买码如果有用户自定义的是否闲家买码,就用用户自定义的是否闲家买码
    get_v_playerbuyLimit(){
        if(this.customcfg['v_playerbuyLimit']!=null)
        {
          return this.customcfg['v_playerbuyLimit'];
        }
        return this.defaultcfg['v_playerbuyLimit'];
    }
    //获取是否下注限制如果有用户自定义的是否下注限制,就用用户自定义的是否下注限制
    get_v_betLimit(){
        if(this.customcfg['v_betLimit']!=null)
        {
          return this.customcfg['v_betLimit'];
        }
        return this.defaultcfg['v_betLimit'];
    }
    //获取是否下注加倍如果有用户自定义的是否下注加倍,就用用户自定义的是否下注加倍
    get_v_betredoubleLimit(){
        if(this.customcfg['v_betredoubleLimit']!=null)
        {
          return this.customcfg['v_betredoubleLimit'];
        }
        return this.defaultcfg['v_betredoubleLimit'];
    }
    //获取是否禁止搓牌如果有用户自定义的是否禁止搓牌,就用用户自定义的是否禁止搓牌
    get_v_shufflingLimit(){
        if(this.customcfg['v_shufflingLimit']!=null)
        {
          return this.customcfg['v_shufflingLimit'];
        }
