...
import LymjLogic from "../LymjMgr/LymjLogic"; 
...
class Model extends BaseModel{
	...
	initData()
	{
		let lymjLogicInstance = LymjLogic.getInstance();
		if(lymjLogicInstance != null)
		{
			...
			this.handcards=g_deepClone(lymjLogicInstance.roundSettle.handcards);
...
class View extends BaseView{
	...
	initUi()
	{
		ctrl.node_hutype.parent.getChildByName('img_drawgame').active = false;
		...
		if(LymjLogic.getInstance().bViewMode){
		...
		if(this.model.b_drawGame){
			...
			this.ui.img_liuju.getComponent(cc.Animation).on('finished', function () {
				ctrl.nodePlayer1Cards.active = true;
				ctrl.nodePlayer2Cards.active = true;
				ctrl.nodePlayer3Cards.active = true;
				ctrl.nodePlayer4Cards.active = true;
				ctrl.nodePlayer1head.active = true;
				ctrl.nodePlayer2head.active = true;
				ctrl.nodePlayer3head.active = true;
				ctrl.nodePlayer4head.active = true;
				ctrl.node_bg.active = true;
				ctrl.imgBanker.node.active = true;
				ctrl.node_hutype.parent.getChildByName('img_drawgame').active = true;
			}, this)
		}		
		this.showUserInfo();
		this.showShareBtn()
	...
	showfan(logicseatid)
	{
		...
		if(playerSettleInfo.fanggangscore != 0 && playerSettleInfo.fanggangscore !=undefined){
			shui = shui + "  放杠"+playerSettleInfo.fanggangscore;
		}
		if(playerSettleInfo.jiegangscore != 0 && playerSettleInfo.jiegangscore !=undefined){
			shui = shui + "  接杠"+playerSettleInfo.jiegangscore;
		}
		...
		//console.log("playerSettleInfo",playerSettleInfo);
	}
	showTotalloseWin(logicseatid)
	{
		...
		if (playerSettleInfo.winflag == 1) {
			...
			if(playerSettleInfo.hutime==MahjongDef.hutime_zimo) {
				let hutype = this.model.getHuType(this.model.handcards[logicseatid]);
				switch(hutype)
				{
					case MahjongDef.hutype_131:
					hutime = 1;
					break;
					case MahjongDef.hutype_7pairs:
					hutime = 2;
					break;
				}
			}
...
//c, 控制
@ccclass
export default class LymjSettleCtrl extends BaseCtrl {
