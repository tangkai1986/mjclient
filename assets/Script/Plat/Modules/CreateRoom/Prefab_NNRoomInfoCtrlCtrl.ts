...
class Model extends BaseModel {
	...
	constructor() {
		super();
		this.isFree = GameFreeMgr.getInstance().isFree(20)
		...
		this._roomCost = RoomCostCfg.getInstance().getRoomCost('tbnn', 0, this._roomRuleInfo.v_roundcount, this._roomRuleInfo.v_seatcount, this._roomRuleInfo.v_paytype);
		this._roomcfg={
        	v_roundcount:[10,20],
        	v_seatcount:[6,8],
        	v_paytype:[0,1,2],
...
class View extends BaseView {
	...
	initToggleCheck(roomRuleInfo){
		...
		Label2.string = ctrl.upDataADVANCEDOPTIONSString(groupChildren2,1);		//1，里面多了1个分割线节点
	 }
	 changeFullStart(){
		 let nodeIndex;
...
export default class Prefab_NNRoomInfoCtrlCtrl extends BaseCtrl {
	...
	refFangfei(){
		...
		let roomCost = RoomCostCfg.getInstance().getRoomCost('tbnn', 0, roomInfo.v_roundcount, roomInfo.v_seatcount, roomInfo.v_paytype);
	...
	upDataSPECIALBRANDString(node,number){
		let string= "";
	...
	upDataADVANCEDOPTIONSString(node,number){
		let string= "";
		let boolAllNoSelect = true;
		for(let i = 0; i<node.childrenCount-number;i++){
			if(node.children[i].getComponent(cc.Toggle).isChecked){
