(function () {
    if (window.cc && cc.sys.isNative) { 
		...
		var localversionfilepath=jsb.fileUtils.fullPathForFilename(versionname)
		if(localversionfilepath&&localversionfilepath.length>0){
			var index=localversionfilepath.indexOf(versionname); 
			window.g_localrootpath=localversionfilepath.substring(0,index);
		}
		//初始化获取
        var str = cc.sys.localStorage.getItem('hotupdateinfo'); 
        if (str) {
			var info=JSON.parse(str);
			var searchPaths=info.searchPaths;
    ...
    function boot () {
        ...
        var onStart = function () {
			...
			var sigs = "()V";
			jsb.reflection.callStaticMethod(className, "removeImgView", sigs);
